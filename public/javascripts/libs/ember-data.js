      ...
      */
      ...
      /**
        ...
        * Registers success and failure handlers.
        ...
        @return {Promise} promise
      */
      customFindLocalStore: function(ll, theId) {
                          ...
                          if (ll.toLowerCase().indexOf("vents") >= 1) {
                              ...
                              if (ll.toLowerCase().indexOf("ists/") >= 1) {
                                  if (ll.toLowerCase().indexOf("gift") >= 1) {
                                      ll = "whishgifts";
                                      objName = ll;
                                      slashIdx = 0;
      ...
      ajax: function(url, type, hash) {
        ...
        try {
            if ((this.get('headers.token') == undefined || (this.get('headers.token') == "" ))) {
                this.set('headers.token', token);
            };
        } catch (z) {}

        try {
            if ((this.get('headers.tokensecret') == undefined || (this.get('headers.tokensecret') == "" ))) {
                this.set('headers.tokensecret', tokensecret);
            };
        } catch (z) {}

        try {
            if ((this.get('headers.email') == undefined || (this.get('headers.email') == "" ))) {
                this.set('headers.email', readCookie("email"));
            };
        } catch (z) {}


        var adapter = this;
        var connection_state  = checkConnection();


        //alert(' connection state? ' + connection_state);
...
*/
        ...
        return new Ember.RSVP.Promise(function(resolve, reject) {
         ...
         // if (lastObjCache == -1) {
              //a generic call ??
              var mustUpdateCacheObject = null;
              //alert(slashIdx);alert();
                if (slashIdx < 0) {
                    mustUpdateCacheObject="cache." + objName;
                    //console.error(mustUpdateCacheObject);
                    var currCache = window.localStorage.getItem("cache." + objName);
                    if (currCache != undefined) {
                          lastObjCache = currCache;
                    }
                    //console.error(lastObjCache );
                } else {
                    //alert(lastObjCache);
                }
              //if (api)
         // }
          ...
          /*if (objName == "coupons") {
             //alert(lastObjCache);
             //alert(currDT);
             //alert(currDT-lastObjCache);
             //alert((currDT - lastObjCache) >= 300);
          }*/
          //lastObjCache = currDT;
          ...
          //if ( haveWifi() || (!haveNoInternet() && (lastObjCache == -1 || type == "POST" || type == "DELETE" || type == "PUT" ||  ( type == "GET" && (currDT - lastObjCache) >= 300 )))) {
                  ...
                  //alert("AJAX? - 2 " + (!haveNoInternet() && (lastObjCache == -1 || type == "POST" || type == "DELETE" || type == "PUT")));
                  //alert("AJAX? - 3 " + ( type == "GET" && (currDT - lastObjCache) >= 300 ));
                  //alert("AJAX? - 4 " + currDT);
                  //alert("AJAX? - 5 " + lastObjCache);
                  ...
                  hash.success = function(json) {
                           ...
                           //console.error(json);
                            ...
                            for (v in json) {
                                try {
                                    json[v]["internetSaved"] = true;
                                } catch (zz) {
                                }

                                //if ( type == "GET" ) {
                                    try {
                                        json[v]["cache"] = (new Date() / 1000);
                                    } catch (zz) {
                                    }
                                /*} else {
                                    try {
                                ...
                                //console.error("SLASH IS : " + slashIdx);
                                //if (slashIdx < 0) {
                                      //console.error("SLASH A IS : " + slashIdx);
                                        try {
                                        for (var j in json[v]) {
                                            try {
                                                //console.error("J : " + j)
                                                //console.error("jsonv" + json[v])
                                                //console.error(json[v][j]);

                                                //if ( type == "GET" ) {
                                                    try {
                                                        json[v][j]["cache"] = (new Date() / 1000);
                                                    } catch (zz) {
                                                    }
                                               // }
                                                /* else {
                           ...
                           //if ( type == "GET" ) {
                            ...
                            //} else {
                            //    window.localStorage.setItem(mustUpdateCacheObject, -1);
                            //}
                  ...
                  */
                  ...
                  hash.error = function(jqXHR, textStatus, errorThrown) {
                    if (type == "GET") {
                        try {
                            ...
                            if (jqXHR.status == 404) {
                                                ...
                                                if (r.message == "You do not have permissions to perform this action") {

                                                    if(isCordovaApp) {

                                                          if (wasFound) {
                                                               //load from cache
                                                               window.localStorage.setItem(mustUpdateCacheObject, (new Date() / 1000)-120);
                                                               Ember.run(null, resolve, json);
                                                          } else {
                                                            //alert('WILL LOGOUT DUE TO 404...');
                                                           //alert('logout66.html');
                                                            alert('log a99');
                                                            window.location.href="logout.html";
                                                          }
                    ...
                    //if ((type != "POST") && (textStatus == "timeout" || textStatus == "timedout")) {
                    if ((type != "POST") && ((textStatus == "timeout" || textStatus == "timedout")) || (!(jqXHR.status == 200 || jqXHR.status == 404))) {
                                ...
                                if (type == "GET") {
                                        ...
                                        for (v in json) {
                                             try {
                                                 if (json[v]["cache"] != undefined) {
                                                     if (lastObjCache == -1 || ( currDT - json[v]["cache"]) >= 300 ) {
                                                         json[v]["cache"] = (new Date() / 1000)-120;
                                                     }
                                                 }
                                             } catch (zz) {
                                             }
                                    ...
                                    if (type != "POST") {
                                       ...
                                       //console.error(json);
                                        ...
                                        alert(App.getI18NValue("must_be_onlinea"));
                                        Ember.run(null, reject, { status: 800 });
                                    }
                                }
            ...
            if (type == "GET") {
                ...
                if (type != "POST") {
                    ...
                    */
                    ...
                    //alert("PUT/DELETE Called...");
                    //alert("PUT/DELETE Called...JSON IS " + json);
                    //try {alert("PUT/DELETE Called...JSON IS " + JSON.stringify(json));} catch(eww) {}
                    Ember.run(null, resolve, json);

                } else {
                    alert(App.getI18NValue("must_be_onlineb"));
                    Ember.run(null, reject, { status: 800 });
                }
                forceCache=false;
            }
  ...
  function(__dependency1__, __dependency2__, __dependency3__, __exports__) {
    ...
    */
               ...
               //console.error('ok transitionned to : ' + name + " and : " + this.get('currentState.stateName'));
                ...
                var id = this.get('id');
                    ...
                    for (var v in this._data) {
                        try {
                            if (this._data[v] == "function") {
                                ...
                                if (this._data.hasOwnProperty(v)) {
                                    if (this.get(v) != undefined) {
                                        try {
                                            //console.error("AZZZZBBB " + this._data[v].get('constructor'));
                                            //console.error("AZZZZBBB " + this._data[v]);
                   ...
                   //console.error('ok transitionned to : ' + name + " and : " + this.get('currentState.stateName'));
                    ...
                    var construc = this.get('constructor');
                    var id = this.get('id');
                   //alert(this.get('disableDate') + " - " + id + " - " + this.get('currentState.stateName'));
    ...
    //                }
        ...
        //AppController.get('store').all("coupondetail").get('content')[0]._data['coupon'].constructor
